...
# plot solution 
ggplot(as.data.frame(mds$points), aes(x=as.data.frame(mds$points)[,1], y=as.data.frame(mds$points)[,2], col=Labels))+geom_point(size=1.4)+theme(aspect.ratio = 1)
#points(cohort_distances_mds$points, col="black", pch=c(8,cohort_outline_pch), bty="n", ann=FALSE)
```
```{r MDS concentrations}
data_labels <- c(rep(1,15), rep(2,15), rep(3,15), rep(4,15), rep(1,15), rep(2,15), rep(3,15), rep(4,15))
d <- dist(cbind(vec_red[(1+n*(1-j)):(n+n*(1-j)), (PL_length_red*0+1):(PL_length_red*30)], vec_gr[(1+n*(1-j)):(n+n*(1-j)),(PL_length_gr*0+1):(PL_length_gr*30)], vec_pos[(1+n*(1-j)):(n+n*(1-j)), (PL_length_pos*0+1):(PL_length_pos*30)], vec_neg[(1+n*(1-j)):(n+n*(1-j)), (PL_length_neg*0+1):(PL_length_neg*30)])) #30 is PL_depth_cap) # euclidean distances between the rows
mds <- cmdscale(d,eig=TRUE, k=2) # k is the number of dim
label_1 <- matrix("0 ng/ml", nrow=nimages, ncol=1)
label_2 <- matrix("5 ng/ml", nrow=nimages, ncol=1)
label_3 <- matrix("15 ng/ml", nrow=nimages, ncol=1)
...
vectorized_PLs <- vec_neg[,1:(PL_length_neg*30)]
...
#p_value <- permutation_test(vectorized_PLs[(3*nimages*npatches+1):(4*nimages*npatches), ],   vectorized_PLs[(3*nimages*npatches+1+4*nimages*npatches):(4*nimages*npatches+4*nimages*npatches), ], num.repeats = nrepeats)
p_value <- permutation_test(vectorized_PLs[(2*nsamples+1):(3*nsamples),], vectorized_PLs[(3*nsamples+1):(4*nsamples),])
...
```{r confusion-matrix-multiclass-SVM-3 cobcentrations}
cohort <- "HA"
if (cohort == "Gata6"){
  j <- 1 #if Gata6
} else {
...
cost <- 10
num_folds <- 10
n <- nsamples*(nconcentrations-1) #number of patches per cohort
ntest_data <- n/num_folds
data_labels <- matrix(rep(5,nsamples), rep(15,nsamples), rep(25,nsamples))
rand_perm <-  sample(n)
#vectorized_PLs <- scale(counts[[1]])


#no 0 concentration
vectorized_PLs <- cbind(vec_red[(nsamples+1+n*(1-j)):(n+n*(1-j)), (PL_length_red*0+1):(PL_length_red*30)], vec_gr[(nsamples+1+n*(1-j)):(n+n*(1-j)),(PL_length_gr*0+1):(PL_length_gr*30)],vec_pos[(nsamples+1+n*(1-j)):(n+n*(1-j)),(PL_length_pos*0+1):(PL_length_pos*30)], vec_neg[(nsamples+1+n*(1-j)):(n+n*(1-j)),(PL_length_neg*0+1):(PL_length_neg*30)])
vectorized_PLs <- vectorized_PLs[, colSums(abs(vectorized_PLs)) != 0]
vectorized_PLs <- scale(vectorized_PLs)
...
#no 0 concentration
vectorized_PLs <- cbind(vec_red[(namples+1+n*(1-j)):(n+n*(1-j)), (PL_length_red*0+1):(PL_length_red*30)], vec_gr[(namples+1+n*(1-j)):(n+n*(1-j)),(PL_length_gr*0+1):(PL_length_gr*30)],vec_pos[(namples+1+n*(1-j)):(n+n*(1-j)),(PL_length_pos*0+1):(PL_length_pos*30)], vec_neg[(namples+1+n*(1-j)):(n+n*(1-j)),(PL_length_neg*0+1):(PL_length_neg*30)]) #30 is PL_depth_cap
#vectorized_PLs <- vectorized_PLs[, colSums(abs(vectorized_PLs)) != 0]
#vectorized_PLs <- scale(vectorized_PLs)
...
H <- ggplot(data, aes(x=concentration, y=value, fill=concentration)) +scale_x_discrete(limits=c("0", "5", "15", "25"))+
    geom_boxplot(alpha=0.4)  +
    geom_jitter(color="black", size=0.7, alpha=0.9)+
    theme(
      plot.title = element_text(size=11), aspect.ratio=1, text = element_text(size=18))+
    xlab("actual Dox concentration")+ylab("predicted Dox concentration")+coord_cartesian(ylim=c(0,28))+scale_y_continuous(breaks = c(-5, 0, 5, 10,15,20,25,30))+scale_fill_discrete(breaks=c('0', '5', '15', '25'))+guides(fill=guide_legend(title="Dox concentration"))
```
...
#ggarrange(SVR_G_mean_sc0_30, SVR_H_mean_sc0_30, nrow=1,common.legend = TRUE, legend = "top")
ggarrange(G, H, nrow=1,common.legend = TRUE, legend = "none")
