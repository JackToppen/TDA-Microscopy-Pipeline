...
for (i in 1:length(data_filenames)){
  ...
  for (k in 1:sqrt(npatches)){ #y-axis
    for (j in 1:sqrt(npatches)){ #x-axis
      ...
      green_cells <- all_cells[ which(all_cells[,8]==10), ]    #green cells
      pos_cells <- all_cells[ which(all_cells[,8]==11), ]      #double positive cells
      neg_cells <- all_cells[ which(all_cells[,8]==0), ]      #double negative cells
...
if (save_computations){
  ...
  save(prediction_H,
       ...
       #vec_red, vec_gr, vec_pos, vec_neg, 
