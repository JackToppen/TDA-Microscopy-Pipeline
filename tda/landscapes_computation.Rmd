...
for (i in 1:length(data_filenames)){
  for (k in 1:npatches){
    ...
    for (j in 1:nrow(PDs[[i]][[k]]$pairs[[2]])){
      if (((PDs[[i]][[k]]$pairs[[2]][j,2]-PDs[[i]][[k]]$pairs[[2]][j,1]) >= 1)){
        count <- count+1
      }
    }
...
for (i in 1:length(data_filenames)){
  for (k in 1:npatches){
    Rcount <- 0
    for (j in 1:nrow(PDs[[i]][[k]]$pairs[[2]])){
     if  (((PDs[[i]][[k]]$pairs[[2]][j,2]-PDs[[i]][[k]]$pairs[[2]][j,1]) >  0.5)){
        Rcount <- Rcount+1
      } else {
      }
    }
...
nrepeats <- 10000
print("Permutation test results:")
p_value <- permutation.test.0(unlist(right_PDs[(1+nsamples*3):(nsamples*4)]), unlist((right_PDs[(1+nsamples*7):(nsamples*8)])))
print(sprintf("p-value %f:", p_value))
```
```
