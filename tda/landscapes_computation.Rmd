---
  title: "Computation of persistence landscapes and vectorized persistence landscapes for one type of cells"
  output:
  html_document:
  df_print: paged
---
...
```{r plot-persistence-diagrams, eval=FALSE}
#2 for homology in degree 1; 1 for homology in degree 0
n <- 2 
#Pick a type of cells
#PDs <- PDs_red 
#PDs <- PDs_green
#PDs <- PDs_pos
PDs <- PDs_neg 
...
```{r average-landscapes-for-each-concentration}
tic()
cohorts <- "Gata6"
conc_avg_max_y <- 0
conc_avg_PLs <- list()
...
```{r vectorize-average-landscapes}
vectorized_PLs <- vectorize_landscapes(PLs_avg, PL_depth_cap) 

#vec_red_1 <- vectorized_PLs
#PL_length_red <- landscape_length
#vec_red_0 <- vectorized_PLs
#vec_gr_1 <- vectorized_PLs
#PL_length_gr <- landscape_length
#vec_gr_0 <- vectorized_PLs
#vec_pos_1 <- vectorized_PLs
#PL_length_pos <- landscape_length
#vec_pos_0 <- vectorized_PLs
vec_neg_1 <- vectorized_PLs
PL_length_neg <- landscape_length
#vec_neg_0 <- vectorized_PLs
#vec_gr_neg <- vectorized_PLs
#vec_red_gr <- cbind(vec_red, vec_green)
#vec_red_pos <- vectorized_PLs
```
